...
if($_SERVER["REQUEST_METHOD"] == "GET"){
    $input_chef_id = trim($_GET["id"]);
    $sql = "SELECT * FROM chefs WHERE chef_id = $input_chef_id";
    $result = $mysqli->query($sql);
    foreach ($result as $row){
        $chef_id = $row['chef_id'];
        $chef_name = $row['chef_name'];
        $chef_position = $row['chef_position'];
        $chef_salary = $row['chef_salary'];
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if (empty($input_chef_position)) {
        $chef_position_err = "Пожалуйста, введите Положение";
    } else{
    ...
    if (empty($input_chef_salary)) {
        $chef_salary_err = "Пожалуйста, введите Зарплата";
    } else{
    ...
    if(empty($chef_id_err) && empty($chef_name_err) && empty($chef_position_err) && empty($chef_salary_err)){
        $sql = "UPDATE chefs 
                SET chef_name = '$chef_name', chef_position = '$chef_position',  chef_salary = '$chef_salary' 
...
<div class="wrapper">
    <div class="container-fluid">
        <div class="row">
            <div class="col-md-12">
                <h2 class="mt-5">Обновить повар</h2>
