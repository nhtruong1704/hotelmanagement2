...
if($_SERVER["REQUEST_METHOD"] == "GET"){
    $input_receptionist_id = trim($_GET["id"]);
    $sql = "SELECT * FROM receptionists WHERE receptionist_id = $input_receptionist_id";
    $result = $mysqli->query($sql);
    foreach ($result as $row){
        $receptionist_id = $row['receptionist_id'];
        $receptionist_name = $row['receptionist_name'];
        $receptionist_salary = $row['receptionist_salary'];
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if (empty($input_receptionist_salary)) {
        $receptionist_salary_err = "Пожалуйста, введите Зарплата";
    } else{
    ...
    if(empty($receptionist_id_err) && empty($receptionist_name_err)  && empty($receptionist_salary_err)){
        $sql = "UPDATE receptionists 
                SET receptionist_name = '$receptionist_name',  receptionist_salary = '$receptionist_salary' 
...
<div class="wrapper">
    <div class="container-fluid">
        <div class="row">
            <div class="col-md-12">
                <h2 class="mt-5">Обновить регистратора</h2>
                <p>Пожалуйста, заполните эту форму и отправьте, чтобы добавить запись о регистратора в базу данных.</p>
