...
if($_SERVER["REQUEST_METHOD"] == "GET"){
    $input_housekeeping_id = trim($_GET["id"]);
    $sql = "SELECT * FROM housekeepings WHERE housekeeping_id = $input_housekeeping_id";
    $result = $mysqli->query($sql);
    foreach ($result as $row){
        $housekeeping_id = $row['housekeeping_id'];
        $housekeeping_name = $row['housekeeping_name'];
        $housekeeping_salary = $row['housekeeping_salary'];
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if (empty($input_housekeeping_salary)) {
        $housekeeping_salary_err = "Пожалуйста, введите Зарплата";
    } else{
    ...
    if(empty($housekeeping_id_err) && empty($housekeeping_name_err)  && empty($housekeeping_salary_err)){
        $sql = "UPDATE housekeepings 
                SET housekeeping_name = '$housekeeping_name',  housekeeping_salary = '$housekeeping_salary' 
...
<div class="wrapper">
    <div class="container-fluid">
        <div class="row">
            <div class="col-md-12">
                <h2 class="mt-5">Обновить горничная</h2>
